...
void Boid::update(vector<vector<vector<Boid *>>> *grid, float resolution) {
  const int cols = floor(ofGetWidth() / resolution);
  const int rows = floor(ofGetHeight() / resolution);
  const int col = floor(position.x / resolution);
  const int row = floor(position.y / resolution);
  vector<Boid *> neighbors;

  for (int i = -1; i <= 1; ++i) {
    for (int j = -1; j <= 1; ++j) {
      const int newCol = col + i;
      const int newRow = row + j;
      if (newCol >= 0 && newCol < cols && newRow >= 0 && newRow < rows) {
        vector<Boid *> newNeighbors = (*grid)[newCol][newRow];
        neighbors.insert(neighbors.begin(), newNeighbors.begin(),
                         newNeighbors.end());
      }
    }
  }
